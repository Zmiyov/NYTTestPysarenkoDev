...
public enum BuyLinksAlert {
    ...
    public static func instagramAlertController(buyLinks: [BuyLinkEntity],
                                                rootVC: UIViewController) -> UIAlertController {

        let gramsAlertController = UIAlertController(title: BuyLinksAlert.chooseAction.comandName.localized(),
                                                     message: nil,
        ...
        for buyLink in buyLinks {
            if let nameMarket = buyLink.marketName, let buyLink = buyLink.buyLinkUrl {
                ...
                let openAction = UIAlertAction(title: BuyLinksAlert.buyIn.comandName.localized() + nameMarket,
                                               style: .default) { _ in
